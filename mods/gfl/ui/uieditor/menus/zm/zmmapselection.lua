...
local sort_map_index = function(mapname)
    if mapname == "zm_prototype" or mapname == "zm_asylum" or mapname == "zm_cosmodrome" or mapname == "zm_moon" or
        mapname == "zm_sumpf" or mapname == "zm_temple" or mapname == "zm_theater" or mapname == "zm_tomb" then
        return CoD.CONTENT_ORIGINAL_MAP_INDEX
    end
    if mapname == "zm_factory" or mapname == "zm_castle" or mapname == "zm_island" or mapname == "zm_stalingrad" or
        mapname == "zm_genesis" then
        return CoD.CONTENT_ORIGINAL_MAP_INDEX
    end
...
DataSources.ZMMapCategoriesDataSource = DataSourceHelpers.ListSetup("ZMMapCategoriesDataSource", function(f6_arg0)
    ...
    local current_dlcIndex = CoD.GetMapValue(Engine.DvarString(nil, "ui_mapname"), "dlc_pack",
        CoD.CONTENT_ORIGINAL_MAP_INDEX)

    local add_to_categories = function(mapCategory, dlcIndex)
        return {
            models = {
                text = Engine.Localize("MPUI_MAP_CATEGORY_" .. mapCategory .. "_CAPS"),
                buttonText = Engine.Localize("MPUI_MAP_CATEGORY_" .. mapCategory .. "_CAPS"),
                image = "playlist_map",
                description = Engine.Localize("MPUI_MAP_CATEGORY_" .. mapCategory .. "_DESC")
            },
            properties = {
                category = dlcIndex,
                selectIndex = current_dlcIndex == dlcIndex
            }
        }
    end
    ...
    local add_to_categories_custommap = function(mapCategory, dlcIndex, button_text, desc_text)
        return {
            models = {
                text = Engine.Localize(button_text),
                buttonText = Engine.Localize(button_text),
                image = "playlist_map",
                description = Engine.Localize(desc_text)
            },
            properties = {
                category = dlcIndex,
                selectIndex = current_dlcIndex == dlcIndex
    ...
    local UserMaps = Engine.Mods_Lists_GetInfoEntries(LuaEnums.USERMAP_BASE_PATH, 0,
        Engine.Mods_Lists_GetInfoEntriesCount(LuaEnums.USERMAP_BASE_PATH))
    if UserMaps and #UserMaps > 0 then
...
DataSources.ZMMapListDataSource = DataSourceHelpers.ListSetup("ZMMapListDataSource", function(menu, controller)
        ...
        for i, map in LUI.IterateTableBySortedKeys(mapsTable, checkUniqueID, check_gameModeEnum) do
            local map_models = {
                models = {
                    ...
                    buttonText = LocalizeToUpperString(CoD.StoreUtility
                                                           .PrependPurchaseIconIfNeeded(menu, i, map.mapName)),
                    image = map.previewImage,
                    description = Engine.Localize(CoD.StoreUtility.AddUpsellToDescriptionIfNeeded(menu, i,
                        map.mapDescription)),
                    dlcIndex = map.dlc_pack
                }
            }
            local properties = {
                mapName = i,
                selectIndex = i == currentmap
            }
    ...
    local UserMaps = Engine.Mods_Lists_GetInfoEntries(LuaEnums.USERMAP_BASE_PATH, 0,
        Engine.Mods_Lists_GetInfoEntriesCount(LuaEnums.USERMAP_BASE_PATH))
    if UserMaps and CoD.perController[menu].mapCategory == CoD.CONTENT_MODS_INDEX then
        for i = 0, #UserMaps, 1 do
            if LUI.startswith(UserMaps[i].internalName, "zm_") then
				table.insert(MapList, {
					models = {
						text = UserMaps[i].name,
						buttonText = LocalizeToUpperString(string.sub(UserMaps[i].name, 1, 32)),
						image = UserMaps[i].ugcName,
						description = UserMaps[i].description
					},
					properties = {
						mapName = UserMaps[i].ugcName,
						selectIndex = i == currentmap
					}
				})
            end
...
LUI.createMenu.ZMMapSelection = function(controller)
    ...
    frame.cac3dTitleIntermediary0.FE3dTitleContainer0.MenuTitle.TextBox1.Label0:setText(Engine.Localize(
        "MPUI_CHANGE_MAP_CAPS"))
    ...
    self:AddButtonCallbackFunction(mapCategoriesList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER",
    ...
    self:AddButtonCallbackFunction(mapList, controller, Enum.LUIButton.LUI_KEY_XBA_PSCROSS, "ENTER",
        function(f26_arg0, f26_arg1, f26_arg2, f26_arg3)
