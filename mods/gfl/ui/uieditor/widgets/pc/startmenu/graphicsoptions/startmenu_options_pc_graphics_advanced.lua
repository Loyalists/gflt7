...
local f0_local0 = function (f1_arg0, f1_arg1)
    ...
    f1_arg0:registerEventHandler("dropdown_triggered", function (Sender, Event)
        for f3_local0 = 1, Sender.graphicsList.requestedRowCount, 1 do
            local f3_local1 = Sender.graphicsList:getItemAtPosition(f3_local0, 1)
...
CoD.StartMenu_Options_PC_Graphics_Advanced.new = function (HudRef, InstanceRef)
	local Widget = LUI.UIElement.new()
	if PreLoadFunc then
		PreLoadFunc(Widget, InstanceRef)
	end
	Widget:setUseStencil(false)
	Widget:setClass(CoD.StartMenu_Options_PC_Graphics_Advanced)
	Widget.id = "StartMenu_Options_PC_Graphics_Advanced"
