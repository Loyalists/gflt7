...
//CARPENTER SCRIPT BY TREYARCH, MODIFIED BY FROST ICEFORGE
//
...
function start_carpenter_new( origin )
{
	...
	//Added notify for Frost Iceforge's BO4 carpenter
...
function get_near_boards(windows)
{
	...
	for(j =0;j<windows.size;j++)
	{
		...
		for(i=0;i<players.size;i++)
		{
			...
			if( distancesquared(players[i].origin, origin) <= BOARD_REPAIR_DISTANCE_SQUARED  )
			{
				close = true;
				break;
			}
