...
#using scripts\core\frontend_cam_override;
...
function createExtraCamXCamData( menu_name, localClientNum, extracam_index, target_name, xcam, sub_xcam, xcam_frame )
{
	...
	ARRAY_ADD( menu_data.extra_cams, extracam_data );
}
...
function addMenuExploders( menu_name, localClientNum, exploder )
{
	...
	if ( IsArray( exploder ) )
	{
		...
		{
			ARRAY_ADD( menu_data.exploders, expl );
		}
	}
	else
	{
		ARRAY_ADD( menu_data.exploders, exploder );
	}
}
...
function createCustomCameraMenu( menu_name, localClientNum, camera_function, has_state, custom_open_fn = undefined, custom_close_fn = undefined )
{
	...
	{
		camera_function = &frontend_cam_override::lobby_main;
	}
		...
		else
		{
			if ( ( status === "closed" || stateChange ) && isdefined( menu_index ) )
			{
				...
				if ( GetDvarInt( "ui_execdemo_e3" ) == 1 ) //&& menu_index > 0 )
				{
					while ( menu_index >= 0 )
					{
						DEFAULT( popped, array::pop_front( clientMenuStack, false ) );
						menu_index--;
					}
				}
				else
